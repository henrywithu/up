...
The source data also includes individual reaction lines, rotating satirical card headlines, pull radius, power, shelter radius, contagion, belief, uprising, coins, particle, and audio parameters. The local implementation currently reuses source costs, life, capacity, icons, image art, basic tints, payment weights, and opening talking/living distribution. At 48 citizens, 2 awake, 22 talking, and 24 living produce the observed opening rate of $102.3M/s at 5%. Its social contagion, exact movement decisions, prop mechanics, giant skinning, end cinematics, and the souvenir are still simplified.

## Audio

The source uses an `AudioContext`, decodes 17 AAC files, starts ambient audio after user input, and plays distraction-specific clips. It loops the dance clip, layers priest/church by a one second gap, and pairs the blame intro/crowd clips. Panic music fades in at 1.4s, ducks ambience to 35%, and fades out over 2.6s. The reference also synthesizes UI hovers/clicks, a notice chord, and a countdown tone, and uses separate lose, timeout, win, and celebration clips. Local audio uses the original AACs, ambient and panic envelopes, and simple synthesized UI clicks; notice chords, countdown, spatial response, and complete cue sequencing remain.

## Verification and fidelity gaps

- Confirmed: all 59 valid assets are present; `npm run build` and `git diff --check` pass; title, tutorial step one, game start, opening economy, rail selection, placement, crowd inspection, and a 390 × 844 layout were inspected in a local browser without console errors.
- Source-derived but currently incomplete: full citizen AI, speech bubbles, confetti/coin simulation, spatial prop behaviors, giant skinning/topple, cinematic victory/photo sharing, exact win/loss copy, audio synthesis and fades, and some touch/landscape transitions.
